...
export class CreateRestaurantePage implements OnInit {
  ...
  guardarProducto(id?:any) {
    ...
    if (this.restauranteForm.valid && this.imagenes.length > 0) {
      this.msgError = ""
      if(this.id){
        console.log('de que entro aqui')
        this.puebloService.updateHotel(this.restauranteForm.value, this.id).subscribe(data=>{
        ...
        this.puebloService.createRestaurante(formData).subscribe((data) => {
          console.log(data)
          if (data) {
  ...
  getHotelById(){
    this.puebloService.getServicioById(this.id, 'restaurante').subscribe((data:any) => {
      console.log(data.data)
      this.restauranteForm.reset(data.data)//resetea el formulario y lo rellena con los datos recicbidos
    })
  }
  ...
  getDataForm(){
    const formData = new FormData()
        ...
        formData.append('nombre', this.restauranteForm.value.nombre)
        formData.append('direccion', this.restauranteForm.value.direccion)
        formData.append('contacto_correo', this.restauranteForm.value.contacto_correo)
        formData.append('contacto_tel', this.restauranteForm.value.contacto_tel)
